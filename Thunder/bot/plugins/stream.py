...
def save_meta(msg_id, data):
    with open(METADATA_FILE, "r+") as f:
        db_data = json.load(f)
        db_data[str(msg_id)] = data
        f.seek(0); json.dump(db_data, f, indent=4); f.truncate()

async def fwd_media(m_msg: Message) -> Optional[Message]:
    try:
        try:
            return await m_msg.copy(chat_id=BIN_CHANNEL)
        ...
        if "MEDIA_CAPTION_TOO_LONG" in str(e):
            ...
            try:
                return await m_msg.copy(chat_id=BIN_CHANNEL, caption=None)
            except FloodWait as e:
                await asyncio.sleep(e.value)
                return await m_msg.copy(chat_id=BIN_CHANNEL, caption=None)
...
async def send_channel_links(links: Dict[str, Any], source_info: str, source_id: int, *, target_msg: Optional[Message] = None, reply_to_message_id: Optional[int] = None):
    try:
        text = MSG_NEW_FILE_REQUEST.format(source_info=source_info, id_=source_id, online_link=links['online_link'], stream_link=links['stream_link'])
        if target_msg:
            await target_msg.reply_text(text, disable_web_page_preview=True, quote=True)
        else:
            await StreamBot.send_message(chat_id=BIN_CHANNEL, text=text, disable_web_page_preview=True, reply_to_message_id=reply_to_message_id)
    except FloodWait as e:
        await asyncio.sleep(e.value)
        text = MSG_NEW_FILE_REQUEST.format(source_info=source_info, id_=source_id, online_link=links['online_link'], stream_link=links['stream_link'])
        if target_msg:
...
async def send_link(msg: Message, links: Dict[str, Any]):
    try:
        await msg.reply_text(MSG_LINKS.format(file_name=links['media_name'], file_size=links['media_size'], download_link=links['online_link'], stream_link=links['stream_link']), quote=True, parse_mode=enums.ParseMode.MARKDOWN, disable_web_page_preview=True, reply_markup=get_link_buttons(links))
    except FloodWait as e:
        await asyncio.sleep(e.value)
        await msg.reply_text(MSG_LINKS.format(file_name=links['media_name'], file_size=links['media_size'], download_link=links['online_link'], stream_link=links['stream_link']), quote=True, parse_mode=enums.ParseMode.MARKDOWN, disable_web_page_preview=True, reply_markup=get_link_buttons(links))
    ...
    async def _actual_link_handler(client: Client, message: Message, **handler_kwargs):
        ...
        if len(parts) > 1:
            try:
                ...
                if not 1 <= num_files <= MAX_BATCH_FILES:
                    await reply_user_err(message, MSG_ERROR_NUMBER_RANGE.format(max_files=MAX_BATCH_FILES))
                    return
            except ValueError:
...
@StreamBot.on_message(filters.channel & filters.incoming & (filters.document | filters.video | filters.audio) & ~filters.chat(BIN_CHANNEL), group=-1)
...
async def process_single(bot: Client, msg: Message, file_msg: Message, status_msg: Optional[Message], shortener_val: bool, original_request_msg: Optional[Message] = None, notification_msg: Optional[Message] = None):
    try:
        ...
        links = {"media_name": file_name, "media_size": humanbytes(file_size), "online_link": cdn_url, "stream_link": cdn_url}

        if notification_msg:
            await safe_edit_message(notification_msg, MSG_LINKS.format(file_name=links['media_name'], file_size=links['media_size'], download_link=links['online_link'], stream_link=links['stream_link']), parse_mode=enums.ParseMode.MARKDOWN, disable_web_page_preview=True, reply_markup=get_link_buttons(links))
        ...
        if status_msg: await safe_edit_message(status_msg, f"❌ 同步存储桶失败：{str(e)}")
        await notify_own(bot, MSG_CRITICAL_ERROR.format(error=str(e), error_id=secrets.token_hex(6)))
        return None

async def process_batch(bot: Client, msg: Message, start_id: int, count: int, status_msg: Message, shortener_val: bool, notification_msg: Optional[Message] = None):
    processed = 0
    failed = 0
    links_list = []
